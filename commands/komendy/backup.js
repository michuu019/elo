...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        zapis.fetch(zapisid).then((infozapis) => {
            ...
            const yyyy = utworzenie.getFullYear().toString(),
                mm = (utworzenie.getMonth() + 1).toString(),
                dd = utworzenie.getDate().toString();
            const data = `${yyyy}/${(mm[1]?mm:"0"+mm[0])}/${(dd[1]?dd:"0"+dd[0])}`;
            let embed = new Discord.MessageEmbed()
                .setTitle("Informacje o zapisie")
                .addFields({ name: "ID Serwera", value: infozapis.data.id }, { name: "Nazwa Serwera", value: infozapis.data.name, }, { name: "Zapis utworzono", value: data }, )
                .setColor("RANDOM");
